...
function So({title,image,desc,stamp,id,stitle,links}) {
  const[edit ,setedit] = useState(false)
   ...
   const [image1,setimage1] = useState('')
    ...
    function update1(e){
      e.preventDefault()
       db.collection('Sports').doc(id).set({
         Stitle:stitle1,
         title:title1,
         description:desc1,
         links:art1
 ...
 const upload =(id)=>{
   ...
   uploadtask.on(
     ...
     (err)=>{
       ...
       storage.ref('images-sports').child(image1.name).getDownloadURL().then(imageurl=>{
          db.collection('Sports').doc(id).update({
           image:imageurl
          })
       })
