...
function Ci({title,image,desc,stamp,id,stitle,links}) {
  ...
  const [image1,setimage1] = useState('')
    ...
    function del(){
        if(window.confirm('Are you sure?')){
            db.collection('Cinema').doc(id).delete();
        }    
    }
    const edit1 = () =>{
      setedit(true)
  ...
  function update1(e){
      e.preventDefault()
       db.collection('Cinema').doc(id).set({
         Stitle:stitle1,
         title:title1,
         description:desc1,
         artlink:art1
 ...
 const upload =(id)=>{
   ...
   uploadtask.on(
     ...
     (err)=>{
       ...
       storage.ref('images-cinema').child(image1.name).getDownloadURL().then(imageurl=>{
          db.collection('Cinema').doc(id).update({
           image:imageurl
          })
       })
