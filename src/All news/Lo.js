...
function Lo({title,image,desc,stamp,id,stitle,links}) {
...
function del(){
        ...
        if(window.confirm('Are you sure?')){
            db.collection('Local').doc(id).delete();
        }
}
function update1(e){
      e.preventDefault()
       db.collection('Local').doc(id).set({
         Stitle:stitle1,
         title:title1,
         description:desc1,
       artlink:art1
 ...
 const upload =(id)=>{
   ...
   uploadtask.on(
     ...
     (err)=>{
       ...
       storage.ref('images-local').child(image1.name).getDownloadURL().then(imageurl=>{
