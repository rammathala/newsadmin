...
function Eo({title,image,desc,stamp,id,stitle,links}) {
  ...
  const [image1,setimage1] = useState('')
    ...
    function update1(e){
      e.preventDefault()
       db.collection('Education').doc(id).set({
         Stitle:stitle1,
         title:title1,
         description:desc1,
         artlink:art1
 ...
 const upload =(id)=>{
   ...
   uploadtask.on(
     ...
     (err)=>{
       ...
       storage.ref('images-education').child(image1.name).getDownloadURL().then(imageurl=>{
